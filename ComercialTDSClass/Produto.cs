...
namespace ComercialTDSClass
{
    public class Produto
    {
        ...
        public Produto(int id, string? codBarras, string? descricao, double valorUnit, string? unidadeVenda, Categoria? categoria, double estoqueMinimo, double classeDesconto, byte[]? imagem)
        {
            Id = id;
            CodBarras = codBarras;
            Descricao = descricao;
    ...
    //spimagem,
        ...
        public bool Atualizar()
        {
            ...
            cmd.Parameters.AddWithValue("spclasse_desconto", ClasseDesconto);
            cmd.Parameters.AddWithValue("spimagem", Imagem);
            if (cmd.ExecuteNonQuery() > 0) // se retornar 0 numero de linhas afetas ele fecha conexão e entrega atualizado. 
                // agora se alguma linha for modificada > q 0 ai entrega o valor atualizado
                atualizado = true;
            cmd.Connection.Close();
            return atualizado;
        }
        public static Produto ObterPorId(int id)
        {
            ...
            if (dr.Read())
            {
                produto = new(
                    ...
                    dr.GetDouble(3),
                    dr.GetString(4),
                    Categoria.ObterPorId(dr.GetInt32(5)), // Aqui categorias entra assim porque é um objeto.
                    dr.GetDouble(6),
                    dr.GetDouble(7),
                    (byte[]?)dr.GetValue(8), // cast objeto para matriz byte[]
                    dr.GetDateTime(9),
                    dr.GetBoolean(10)
                );// verificar se esta na mesma sequencia do banco de dados
