...
namespace ComercialTDSClass
{
    public class Pedido
    {
        ...
        public bool Atualizar()//pq bool pra esse metodo?: porqe se for true executa a mudança se for false nao muda
        {
            var cmd = Banco.Abrir();
            cmd.CommandType = System.Data.CommandType.StoredProcedure;
        ...
        public static Pedido ObterPorId(int id) // porque o nome da classe veio antes pra gerar esse metodo? porque n preciso declarar o objeto pedido lá fora para usa-lo não precisa ser criado um objeto
        {
            Pedido pedido = new();
            var cmd = Banco.Abrir();
    ...
    public static List<Pedido> ObterLista()
        {
            ...
            var cmd = Banco.Abrir();
            cmd.CommandText = $"select * from pedidos"; //limitar a lista q vai aparecer
            var dr = cmd.ExecuteReader();
            while (dr.Read())
            {
                pedidos.Add(new(
